  ...
  try {
    xhr.onreadystatechange = function() {
      ...
      if (xhr.responseXML) {
        ...
        var latestEntrySet = xmlDoc.evaluate("/gmail:feed/gmail:entry",
            xmlDoc, gmailNSResolver, XPathResult.ANY_TYPE, null);
        var latestEntryNode = latestEntrySet.iterateNext();

        localStorage.newEntryId = latestEntryNode.childNodes[5].textContent;
        var latestEntryId;
        var latestEntryTitle;
        var latestEntrySummary;
        var latestEntryAuthor;

        if (localStorage.newEntryId === localStorage.latestEntryId) {
          console.log("nothing to notify");
        } else {
          console.log("a new email found!");
          localStorage.latestEntryId = localStorage.newEntryId;
          latestEntryTitle = latestEntryNode.childNodes[0].textContent;
          latestEntrySummary = latestEntryNode.childNodes[1].textContent;
          latestEntryAuthor = latestEntryNode.childNodes[6].childNodes[0].textContent;

          notify(latestEntryAuthor, latestEntryTitle, latestEntrySummary);
        }
...
function notify(author, title, summary) {
  if (!title) {
    title = 'No title';
  }
  chrome.notifications.create(notificationId, {
    ...
    type: 'basic',
    message: title,
    buttons: [{ title: 'Liked' }, { title: 'Disliked'}],
    isClickable: true,
    priority: 2,
  });
}
